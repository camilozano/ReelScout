...
*   **Downloader Media Type Handling (19 Apr 2025):**
    *   Modified `src/downloader.py::download_collection_media` to handle `media_type` 1 (Photo) and 8 (Carousel) in addition to 2 (Video).
    *   Removed the explicit check that skipped non-video types.
    *   Added logic to use `client.photo_download` for `media_type=1`.
    *   Added logic for `media_type=8`: Creates a subdirectory named after the carousel's PK, stores the relative subdirectory path (e.g., `{pk}/`) in the main metadata's `relative_path`, iterates through `media.resources`, and downloads each resource (photo/video) into the subdirectory using the appropriate download function (`photo_download` or `video_download`). Includes checks for existing resource files within the subdirectory.
    *   Updated logging and summary counts.
    *   Updated `tests/test_downloader.py` with new fixtures and test cases (`test_download_success_mixed_types`, `test_download_skip_existing_photo_file`, `test_download_skip_existing_carousel_subdir`, `test_download_skip_existing_carousel_resource`, `test_download_carousel_empty_resources`, `test_download_carousel_resource_download_error`, etc.) to cover photo and carousel handling, including success, skipping existing files/subdirs/resources, and the `--skip-download` flag. Modified existing tests to accommodate the broader media type handling.
